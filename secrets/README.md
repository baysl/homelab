...
4. To allow Flux to decrypt the secrets during reconciliation, ensure a Kustomization resource is created in the `flux-system` namespace that references the SOPS decryption secret:
    ...
    decryption:
      ...
      secretRef:
         name: sops-age
    ```
    Check [`clusters/homelab/secrets-kustomization.yaml`](https://github.com/baysl/homelab/blob/main/clusters/homelab/secrets-kustomization.yaml) to see how this is implemented in this repository.
